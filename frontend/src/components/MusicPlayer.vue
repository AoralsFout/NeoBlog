...
// ========== 状态缓存 ==========
const MUSIC_STATE_KEY = 'neo-blog-music-player-state'

interface MusicPlayerCache {
    volume: number
    playMode: PlayMode
    currentMusicId: number | null
    currentTime: number
    isMusicBoxOpen: boolean
    isPaused: boolean
    isTranslate: boolean
}
...
const saveMusicState = () => {
    try {
        const state: MusicPlayerCache = {
            volume: volume.value,
            playMode: playMode.value,
            currentMusicId: currentMusic.value?.id ?? null,
            currentTime: currentTime.value,
            isMusicBoxOpen: isMusicBoxOpen.value,
            isPaused: audio.value?.paused ?? true,
...
const loadMusicState = (): MusicPlayerCache | null => {
    try {
...
const resumePaused = ref<boolean | null>(null)
...
const loadMusicList = async () => {
    try {
        ...
        // 尝试从缓存恢复状态
        const cached = loadMusicState()
        if (cached) {
            volume.value = cached.volume
            playMode.value = cached.playMode
            isTranslate.value = cached.isTranslate
            resumePaused.value = cached.isPaused

            if (cached.currentMusicId) {
                const index = musicList.value.findIndex(m => m.id === cached.currentMusicId)
                if (index !== -1) {
                    if (cached.currentTime > 0) {
                        resumeTime.value = cached.currentTime
                    }
                    switchMusic(index)
                    // 恢复音乐盒展开状态
                    if (cached.isMusicBoxOpen) {
                        toggleMusicBox()
                    }
                    return
                }
...
const handleLoadedMetadata = () => {
    ...
    if (resumeTime.value !== null) {
        audio.value.currentTime = resumeTime.value
        currentTime.value = resumeTime.value
        resumeTime.value = null
    }
    // 如果之前是暂停状态，恢复后保持暂停
    if (resumePaused.value === true && audio.value) {
        audio.value.pause()
    }
...
const handleTimeUpdate = () => {
    ...
    translatedLyricsData.value.forEach((lyric, index) => {
        if (audio.value && (audio.value.currentTime >= lyric.time && audio.value.currentTime < (translatedLyricsData.value[index + 1]?.time || duration.value))) {
